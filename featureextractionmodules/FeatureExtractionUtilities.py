...
class FeatureExtractionUtilities:
    ...
    neg_feature_vecs = numpy.zeros((100, 200), dtype="float32")
    neg_model = None
    pos_feature_vecs = numpy.zeros((100, 200), dtype="float32")
    pos_model = None
    goodwords = []
    badwords = []
    lesswords = []
    ...
    def loadItems():
        ...
        for line in infile:
            if not line[0] == ';':
                bingnegs.append(stemmer.stem((line.decode('utf8', 'ignore').encode('ascii', 'ignore')).strip()))
        infile = open('./sentimentanalysisresources/bingliuposs.txt')
        for line in infile:
            if not line[0] == ';':
                bingposs.append(stemmer.stem((line.decode('utf8', 'ignore').encode('ascii', 'ignore')).strip()))
    ...
    def generateCentroidSimilarityScore(sent):
        ...
        averagevec = numpy.zeros((300,), dtype="float32")
        for t in terms:
            try:
                averagevec = numpy.add(averagevec, FeatureExtractionUtilities.neg_model[t.lower()])
            except KeyError:
                pass
        try:
    ...
    def getsentiwordscores(processed_data):
        ...
        for d in processed_data:
            ...
            for i in range(0, len(pos_tags)):
                try:
                    if str(pos_tags[i][1]).lower() == 'adj':
                        if FeatureExtractionUtilities.sentiposscores.has_key((str(pos_tags[i][0]).lower(), 'a')):
                            posscore += float(
                                FeatureExtractionUtilities.sentiposscores[(str(pos_tags[i][0]).lower(), 'a')])
                        if FeatureExtractionUtilities.sentinegscores.has_key((str(pos_tags[i][0]).lower(), 'a')):
                            negscore += float(
                                FeatureExtractionUtilities.sentinegscores[(str(pos_tags[i][0]).lower(), 'a')])
                    if str(pos_tags[i][1]).lower() == 'verb':
                        if FeatureExtractionUtilities.sentiposscores.has_key((str(pos_tags[i][0]).lower(), 'v')):
                            posscore += float(
                                FeatureExtractionUtilities.sentiposscores[(str(pos_tags[i][0]).lower(), 'v')])
                        if FeatureExtractionUtilities.sentinegscores.has_key((str(pos_tags[i][0]).lower(), 'v')):
                            negscore += float(
                                FeatureExtractionUtilities.sentinegscores[(str(pos_tags[i][0]).lower(), 'v')])
                    if str(pos_tags[i][1]).lower() == 'noun':
                        if FeatureExtractionUtilities.sentiposscores.has_key((str(pos_tags[i][0]).lower(), 'n')):
                            posscore += float(
                                FeatureExtractionUtilities.sentiposscores[(str(pos_tags[i][0]).lower(), 'n')])
                        if FeatureExtractionUtilities.sentinegscores.has_key((str(pos_tags[i][0]).lower(), 'n')):
                            negscore += float(
                                FeatureExtractionUtilities.sentinegscores[(str(pos_tags[i][0]).lower(), 'n')])
                except Exception:
                    pass
    ...
    def getSynsetString(sent, negations):
        ...
        for i in range(0, len(pos_tags)):
            ...
            if str(pos_tags[i][1]).lower() == 'adj':
                synsets = wn.synsets(pos_tags[i][0].lower(), pos=wn.ADJ)
                for syn in synsets:
                    lemmas = [lemma.lower() for lemma in syn.lemma_names()]
                    sent_terms += lemmas

            if pos_tags[i][1].lower() == 'verb':
                synsets = wn.synsets(pos_tags[i][0].lower(), pos=wn.VERB)
                for syn in synsets:
                    lemmas = [lemma.lower() for lemma in syn.lemma_names()]
                    sent_terms += lemmas

            if pos_tags[i][1].lower() == 'noun':

                synsets = wn.synsets(pos_tags[i][0].lower(), pos=wn.NOUN)
                for syn in synsets:
                    lemmas = [lemma.lower() for lemma in syn.lemma_names()]
    ...
    def detectModals(senttokens, modals):
        for word in senttokens:
            for item in modals:
                if ((item.lower()>word.lower())-(item.lower()<word.lower())) == 0:
                    return 1
        return 0
    ...
    def loadGoodWords():
        ...
        for line in infile:
            goodwords.append(stemmer.stem(line.strip()))
    ...
    def loadBadWords():
        ...
        for line in infile:
            badwords.append(stemmer.stem(line.strip()))
    ...
    def loadMoreWords():
        ...
        for line in infile:
            morewords.append(stemmer.stem(line.strip()))
    ...
    def loadLessWords():
        ...
        for line in infile:
            lesswords.append(stemmer.stem(line.strip()))
    ...
    def goodbadFeatures(processed_data):
        goodbadfeatures = []
        for pandas in processed_data:

            moreGood = 0
            moreBad = 0
            lessGood = 0
            lessBad = 0
            sentence = pandas.lower()
